	...
	//this file is copied of webx.js 
	( function ( global_name ) {

		self.webextension_library_name = global_name;
		self[ global_name ] = {};

	} ( "webextension_library" ) );
	self[ self.webextension_library_name ].util = ( function () {
		
		// var parser = new DOMParser();
		var x = self[ self.webextension_library_name ];
		...
		return {
			...
			// Function to request HTML parsing in content script
			parse_html_in_content_script: function (html) {
				return new Promise((resolve) => {
					chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
						chrome.tabs.sendMessage(tabs[0].id, { action: "parseHTML", html: html }, (response) => {
							resolve(response ? response.document : null);
						});
					});
				});
			},
			...
			text_to_doc: function ( text ) {
				...
				return this.parse_html_in_content_script( text );
			...
			html_to_doc: function ( html ) {
				...
				return this.parse_html_in_content_script( html );
	...
	self[ self.webextension_library_name ].procedures = ( function () {
	...
	self[ self.webextension_library_name ].logs = ( function () {
	...
	self[ self.webextension_library_name ].url = ( function () {

		return {
		};

	} () );
	self[ self.webextension_library_name ].hub = function ( mode, options ) {
		...
		return {
			...
			send_runtime_message_rq_to_rs: function ( rq ) {
				...
				self.chrome.runtime.sendMessage( rq );
			...
			send_tab_message_rq_to_rs: function ( req ) {
				...
				if ( req.all_tabs_flag ) {
					...
					self.chrome.tabs.query( {}, function ( tab_arr ) {

						tab_arr.forEach( function ( tab ) {

							self.chrome.tabs.sendMessage( tab.id, req );
					...
					return new Promise( function ( resolve ) {
						...
						self.chrome.tabs.sendMessage( req.tab_id, req, resolve );
			...
			add_window_observers: function ( self, context, sender, observers ) {
			...
			message_window: function ( self, context, sender, name, data ) {
	...
	self[ self.webextension_library_name ].log = ( function () {
	...
	self[ self.webextension_library_name ].report_manager_hub = ( function () {
	...
	self[ self.webextension_library_name ].report_manager = ( function () {
	...
	self[ self.webextension_library_name ].tester = ( function () {
		...
		return {
			...
			html_to_element: function ( html ) {
				...
				// var parser = new DOMParser;
				var dom = this.parse_html_in_content_script( html );
	...
	self[ self.webextension_library_name ].ajax = ( function () {
	...
	self[ self.webextension_library_name ].ajax2 = ( function () {
	...
	self[ self.webextension_library_name ].query = ( function () {
	...
	self[ self.webextension_library_name ].storage = ( function () {
		...
		return {
			...
			get: function ( path ) {
				...
				return new Promise ( function ( resolve ) {
					...
					self.chrome.storage.local.get( null, function ( items ) {
			...
			set: function ( path, value ) {
				...
				return new Promise ( function ( resolve ) {
					...
					self.chrome.storage.local.get( null, function ( items ) {
						...
						self.chrome.storage.local.set( items, resolve.bind( this, true ) );
	...
	self[ self.webextension_library_name ].detect = ( function () {
	...
	self[ self.webextension_library_name ].bg_api = ( function () {
		...
		return {
			...
			exec: function ( api_name, method_name, input ) {
				...
				if ( chrome.extension.getBackgroundPage && chrome.extension.getBackgroundPage() === self ) {
	...
	self[ self.webextension_library_name ].conv = ( function () {
	...
	self[ self.webextension_library_name ].convert = ( function ( input, data_arr ) {
	...
	self[ self.webextension_library_name ].chrome_p = ( function () {
	...
	self[ self.webextension_library_name ].cache_manager = function () {
